<script setup lang="tsx">
	...
	const columns: NaiveUI.TableColumn<Api.Tools.GeneratorTableColumnEdit>[] = [
		...
		},
		{
			...
			align: 'center',
			width: 200,
			render: (row: TableColumnModel) => <NSelect v-model:value={row.dictCode} size='small' filterable clearable options={tableDictOptions.value}></NSelect>
		},
		{
			key: 'status',
...
<template>
	<NModal v-model:show="visible" :title="title" preset="card" class="h-[80vh] w-4/5">
		<NTabs v-model:value="tabIndex" type="line" animate size="small" class="h-full">
			...
			<NTabPane :name="2" :tab="$t('page.tools.generateTableColumn.columnInfo')" class="h-full flex flex-col">
				<NSpace>
			...
			<NTabPane :name="3" :tab="$t('page.tools.generateTableColumn.resultInfo')" class="flex flex-1 items-center justify-center">
