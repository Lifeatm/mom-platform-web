...
import { jsonClone } from '@lifeatm/utils'
import { useBoolean, useHookTable } from '@lifeatm/hooks'
...
export function useTableOperate<T extends TableData = TableData>(data: Ref<T[]>, getData: () => Promise<void>) {
	 ...
	 * @param key 字段名
	 */
	function getCheckedFieldValues<K extends keyof T>(key: K): Array<T[K]> {
